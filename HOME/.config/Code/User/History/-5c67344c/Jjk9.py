...
def translate_live_audio(speech_key, speech_region, source_language, target_language,
    ...
    """
    Uses the system microphone for continuous recognition and translation.
    As segments are recognized, the final recognized text and its translation are
    enqueued to the provided queues.

    Parameters:
        speech_key (str): Azure Speech service subscription key.
        speech_region (str): Azure region (e.g., "westus").
        source_language (str): Source language in BCP-47 format (e.g., "en-US").
        target_language (str): Target language code for translation (e.g., "fr").
        recognized_text_queue (queue.Queue): Queue to store recognized text segments.
        translated_text_queue (queue.Queue): Queue to store translated text segments.
        stop_event (threading.Event): Event to signal when to stop recognition.
        done_event (threading.Event): Event that is set when recognition stops.
    """
