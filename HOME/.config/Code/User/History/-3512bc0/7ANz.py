...
def synthesize_voice(input_text, language):
    """
    Synthesizes the given input_text into speech and plays it through the default speaker.
    """
    if not speech_key or not service_region:
    ...
    # Instead of saving to a file, configure the synthesizer to use the default speaker.
    ...
    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        print("[Synthesis] Audio played successfully.")
    else:
        print("Speech synthesis canceled or failed.")

def pipeline_worker(segment_queue, target_language):
    """
    Worker thread that processes each segment:
      - Translates the text.
      - Synthesizes the translated text and plays it on the default speaker.
    """
    while True:
        ...
        try:
            ...
            print("[Pipeline] Synthesizing and playing audio...")
...
if __name__ == "__main__":
    ...
    worker_thread = threading.Thread(target=pipeline_worker, args=(segment_queue, target_language))
    worker_thread.start()

    start_time = time.time()

    # Begin transcription; segments will be pushed into the queue as they are recognized.
