...
def transcribe_audio_to_queue(file_path, segment_queue):
    ...
    auto_detect_source_language_config = speechsdk.languageconfig.AutoDetectSourceLanguageConfig(
        languages=["en-US", "sr-Latn", "fr-FR", "it-IT"]
    )
...
def synthesize_voice(input_text, language, audio_queue):
    ...
    print(f"[Synthesis] Synthesis result: {result.reason} \n{input_text}")
    ...
    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        ...
        fmt = result.properties.get_property(speechsdk.PropertyId.SpeechServiceResponse_JsonResult)
    else:
