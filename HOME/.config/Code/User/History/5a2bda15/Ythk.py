...
def pick_voice_interface(language_prefix, speech_key, speech_region):
    """
    Lists available voices for the given language prefix (e.g., "en") and prompts the user
    to select one by number.

    Parameters:
        language_prefix (str): Language prefix (e.g., "en" to list all English voices).
        speech_key (str): Azure Speech service subscription key.
        speech_region (str): Azure Speech service region (e.g., "westus").

    Returns:
        str: The name of the selected voice (e.g., "en-US-AriaNeural") or None if no voice found.
    """
    # Configure the Speech SDK for synthesis.
    speech_config = speechsdk.SpeechConfig(subscription=speech_key, region=speech_region)
    # Although you could set speech_synthesis_language here, we'll filter manually.
    # Create a synthesizer (without audio output) to retrieve the voices.
    ...
    # Filter voices to include only those whose locale starts with the provided language prefix.
    filtered_voices = [
        voice for voice in all_voices 
        if voice.locale.lower().startswith(language_prefix.lower())
    ]

    if not filtered_voices:
        print(f"No voices available for language prefix '{language_prefix}'.")
        return None

    print(f"Available voices for language prefix '{language_prefix}':")
