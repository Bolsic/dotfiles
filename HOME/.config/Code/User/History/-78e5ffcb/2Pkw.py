...
def synthesize_voice(input_text, language, output_path, speech_config):
    """
    Synthesizes input_text using a voice for the given target language.
    """
    ...
    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        ...
        print("[Synthesis] Speech synthesis failed.")
...
def push_stream_worker(file_path, push_stream):
    ...
    """
    chunk_size = 3200  # Suitable for 16kHz, 16-bit mono audio.
    try:
    ...
    try:
        while True:
            ...
            if not data:
                break
            print(f"[Push] Read {len(data)} bytes.")
            push_stream.write(data)
            time.sleep(0.1)
        print("[Push] Finished reading file; waiting before closing stream...")
        time.sleep(1.0)  # Allow final audio data to be processed.
    finally:
        wf.close()
        push_stream.close()
        print("[Push] Push stream closed.")

def transcribe_audio(push_stream, segment_queue, speech_config):
    """
    Uses a PushAudioInputStream to feed real-time audio to the recognizer.
    Recognized segments are enqueued as (text, detected_language) tuples.
    """
    ...
    segment_queue.put(None)  # Sentinel to indicate end of segments.

def pipeline_worker(segment_queue, target_language, base_output_path, speech_config):
    """
    For each transcription segment, writes out the transcribed text,
    translates it, writes out the translated text, and synthesizes the speech.
    """
    segment_index = 0
    while True:
        ...
        try:
            # Write out the transcribed text.
            transcribed_filename = f"{base_output_path}_segment_{segment_index}_transcribed.txt"
            with open(transcribed_filename, "w") as f:
                f.write(transcription_text)
            print(f"[Pipeline] Transcribed chunk written to: {transcribed_filename}")
            ...
            translated_filename = f"{base_output_path}_segment_{segment_index}_translated.txt"
            with open(translated_filename, "w") as f:
                f.write(translated_text)
            print(f"[Pipeline] Translated chunk written to: {translated_filename}")

            # Synthesize the translated text into an audio file.
            segment_audio_path = f"{base_output_path}_segment_{segment_index}.wav"
            print(f"[Pipeline] Synthesizing segment {segment_index} to {segment_audio_path}...")
            synthesize_voice(
