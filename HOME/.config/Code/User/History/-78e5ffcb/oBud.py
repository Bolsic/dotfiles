...
def transcribe_audio_to_queue(file_path, segment_queue, speech_config):
    """
    This function sets up continuous recognition. In the recognized callback,
    each recognized phrase (with its detected language) is placed into the segment_queue.
    """
    if not speech_key or not service_region:
        print("Environment variables for Azure Speech service are not set.")
        return

    # Set up auto-detect languages (up to 4)
    ...
    def recognized_callback(evt):
        if evt.result.reason == speechsdk.ResultReason.RecognizedSpeech:
            # Get the detected language from the result properties
...
def synthesize_voice(input_text, language, output_path, speech_config):
    ...
    # Create a new SpeechConfig (or modify the global one) for synthesis
    synthesis_config = speech_config
...
def pipeline_worker(segment_queue, target_language, base_output_path):
    """
    This worker runs in a separate thread. It continuously pulls segments from the queue.
    For each segment, it translates and synthesizes the audio into a separate file.
    """
    segment_index = 0
    while True:
        item = segment_queue.get()
        if item is None:
            # Sentinel value indicates no more segments
        ...
        try:
            ...
            # Create an output file name for this segment
...
if __name__ == "__main__":
    # Define the directory path for the input audio file.
    ...
    # Start transcription in the main thread (or you could also start it in its own thread).
    start_time = time.time()
    transcribe_audio_to_queue(audio_file_path, segment_queue, speech_config)
