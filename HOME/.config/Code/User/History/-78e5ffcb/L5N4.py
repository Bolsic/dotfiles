...
def make_output_dir(input_path):
    """
    Generates a base output path by replacing "Original-Audio" with "Translated-Audio"
...
def push_stream_worker(file_path, push_stream):
    ...
    Adds a final delay before closing the stream.
    """
    chunk_size = 3200  # Adjust as needed for 16kHz, 16-bit mono audio.
    try:
    ...
    try:
        while True:
            ...
            time.sleep(0.1)
        # Allow some extra time for any final processing.
...
def transcribe_audio(push_stream, segment_queue, speech_config):
    """
    Feeds audio from a PushAudioInputStream into the recognizer.
    ...
    done_event.wait()  # Wait until the session stops (end-of-stream)
...
def pipeline_worker(segment_queue, target_language, base_output_path, speech_config):
    """
    For each transcription segment, translates and synthesizes the result.
    Each segment is saved to a separate output file.
    """
    segment_index = 0
    while True:
...
if __name__ == "__main__":
    # Choose the input file and target language.
    ...
    # Create a push stream for streaming audio data.
    push_stream = speechsdk.audio.PushAudioInputStream()

    # Start the push stream worker to read the file and feed the push stream.
    push_thread = threading.Thread(target=push_stream_worker, args=(audio_file_path, push_stream))
    push_thread.start()

    # Start the transcription thread that uses the push stream.
    transcription_thread = threading.Thread(target=transcribe_audio, args=(push_stream, segment_queue, speech_config))
    transcription_thread.start()

    # Start the pipeline worker thread to translate and synthesize segments.
